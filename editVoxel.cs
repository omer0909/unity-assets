...
public class editVoxel : MonoBehaviour
{
    ...
    private List<voxelMesh> mesh=new List<voxelMesh>();
        ...
        for(int x=0;x<distanceInt*2+1;x++){
            for(int y=0;y<distanceInt*2+1;y++){
                ...
                if(!positions.Contains(posCreated) && Vector2.Distance(pos-new Vector2(size*0.5f,size*0.5f),posCreated*size)<renderDistance){
                    ...
                    mesh.Add(created.GetComponent<voxelMesh>());
                }
    ...
    void edit(Vector3Int pos){
        ...
        voxelMesh meshMain=mesh[positions.IndexOf(index)];
        ...
        if(indexpos.x==size-1 && positions.Contains(index+Vector2Int.right)){
            ...
            voxelMesh meshlook= mesh[positions.IndexOf(index+Vector2Int.right)];
            ...
            voxelMesh meshlook= mesh[positions.IndexOf(index+Vector2Int.left)];
        ...
        if(indexpos.z==size-1 && positions.Contains(index+Vector2Int.up)){
            ...
            voxelMesh meshlook= mesh[positions.IndexOf(index+Vector2Int.up)];
            ...
            voxelMesh meshlook= mesh[positions.IndexOf(index+Vector2Int.down)];
